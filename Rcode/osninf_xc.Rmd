...
```{r}
least_individual=which(Fit.MM$scores[,1] %in% Fit.MM$scores[,1][order(Fit.MM$scores[,1],decreasing = TRUE)[1:5]])
least_individual
```
...
least_individual_10=which(Fit.MM$scores[,1] %in% Fit.MM$scores[,1][order(Fit.MM$scores[,1],decreasing = TRUE)[1:number_10_percent]])
least_individual_10
```
...
```{r}
most_individual=which(Fit.MM$scores[,1] %in% Fit.MM$scores[,1][order(Fit.MM$scores[,1])[1:5]])
most_individual
```
...
osninf_compare=function(path_for_csv_fiels,number_top_users){
#   #read the data
...
# A_sampled_adoption_curve=read.csv("/Users/xzhao17/Documents/GitHub/osninf/newdata/sampled_1D_user_adoption_curve.csv")
  ...
  legend("topright",legend = c("adoption curve","predicted curve from individuals"),lty=c(1,3),col=
         c("blue","red"))
  ###repeat the same for the 2nd metric
  
  
  
  ###repeat the same for the 3rd metric
