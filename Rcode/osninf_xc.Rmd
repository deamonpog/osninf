...
adoption_volumn_fit=function(adoption_or_volumn_data,main_text){
...
plot(as.Date(adoption_or_volumn_data$created_at),adoption_or_volumn_data$Count,xlab="",
     ylab="Count",cex.lab=1.5,cex.axis=1.5,main=main_text)
...
get_fpca_sc_result=function(Matrix_RRCount_per_period,number_top_users,
  ...
  legend("top",legend = c("Adoption Curve","Predicted Curve from Individuals"),lty=c(1,3),col=
         c("blue","red"),inset = c(0,-0.35))
...
```{r}
#c("Adoption Curve","Mean Total Transfer Entrophy for Top 10% Users","Individual Total Transfer Entrophy")
get_top_percent_user_results=function(Matrix_RRCount_per_period,top_individual,
                                      ...
                                      adoption_time,legend_text){
  #legend_text=c("1","3","4")
  #adoption_smooth_curve=smooth_adoption_curve
  ...
  ##raw curve
  # 
  # matplot(as.Date(Matrix_RRCount_per_period$created_at),   data_change_na[,-1],xlab="",ylab="Value",cex.lab=1.5,cex.axis=1.5,type="l")
  ###############
  
  top_mean_curve=apply(data_change_na[,c(top_individual)],1,mean,na.rm=TRUE)
  
  temp_data=Matrix_RRCount_per_period[,-1]
  y_lim_upper=max(temp_data[,c(top_individual)],adoption_smooth_curve,na.omit(top_mean_curve))
               
  par(mar=c(5, 4, 5, 5), xpd=TRUE)
  matplot(as.Date(Matrix_RRCount_per_period$created_at),data_change_na[,c(top_individual)],xlab="",ylab="Count",
          type = "l",cex.lab=1.5, cex.axis=1.5,lty=1, lwd=3,ylim=c(0,y_lim_upper),col="grey")
  lines(as.Date(Matrix_RRCount_per_period$created_at),top_mean_curve,xlab="",ylab="Count",
          type = "l",cex.lab=1.5, cex.axis=1.5,lty=1, lwd=3,ylim=c(0,y_lim_upper),col="red")
  ...
  # legend("topright",legend=legend_text,lty=c(1,1),col=
  #          c("blue","red","grey"))
   legend("top",legend=legend_text,lty=c(1,1),col=
           c("blue","red","grey"),inset = c(0,-0.35))
...
# #          c("blue","red"))
# legend("top",legend = c("Adoption Curve","Mean Total Transfer Entrophy Curve"),lty=c(1,1),col=
#          c("blue","red"),inset = c(0,-0.3))
```
...
osninf_compare=function(path_for_datasets,dataset_name,first_few_days,percentage_threshold,
                        Matrix_Measure_per_period,Measure){
#   #read the data
...
# A_sampled_adoption_curve=read.csv("/Users/xzhao17/Documents/GitHub/osninf/newdata/sampled_1D_user_adoption_curve.csv")
  ...
  ###adoption curve
  main_text="Adoption Curve"
  adoption_curve=adoption_volumn_fit(A_sampled_adoption_csv,main_text)
  adoption_smooth_curve=adoption_curve$smooth_curve
  adoption_time=adoption_curve$Time01
  ##volumn curve
  main_text="Volumn Curve"
  volumn_curve=adoption_volumn_fit(volume_csv,main_text)
  ...
  TTE_fpca_results=get_fpca_sc_result(Matrix_Measure_per_period,number_top_users,
                            adoption_time,adoption_smooth_curve)
  if (Measure=="RR_Count"){
     legend_text=c("Adoption Curve","Mean RR Count for Top High Variability Users","Individual RR Count")
  }else{
    legend_text=c("Adoption Curve","Mean Total Transfer Entrophy for Top High Variability Users","Individual Total Transfer Entrophy")
  }
  ...
  TTE_fpca_high_varability_user_results=get_top_percent_user_results(Matrix_Measure_per_period,
                                      ...
                                      adoption_time,legend_text)
  if (Measure=="RR_Count"){
     legend_text=c("Adoption Curve","Mean RR Count for Top Low Variability Users","Individual RR Count")
  }else{
    legend_text=c("Adoption Curve","Mean Total Transfer Entrophy for Top Low Variability Users","Individual Total Transfer Entrophy")
  }
  ...
  TTE_fpca_low_varability_user_results=get_top_percent_user_results(Matrix_Measure_per_period,
                                      ...
                                      adoption_time,legend_text)
  if (Measure=="RR_Count"){
     legend_text=c("Adoption Curve","Mean RR Count for Top 10% Sum Users","Individual RR Count")
  }else{
    legend_text=c("Adoption Curve","Mean Total Transfer Entrophy for Top 10% Sum Users","Individual Total Transfer Entrophy")
  }
  ...
  if (Measure=="RR_Count"){
     legend_text=c("Adoption Curve","Mean RR Count for First 10% Users","Individual RR Count")
  }else{
    legend_text=c("Adoption Curve","Mean Total Transfer Entrophy for First 10% Users","Individual Total Transfer Entrophy")
  }
...
print_column_plot=function(data_frame,main_text){
  zero_percent=c(0)
    for (i in 1:dim(data_frame)[2]){
      #   cat("Data for User:\t", i,
      # "\n \t")
      #  print(data_frame[,i])
    zero_percent[i]=   (sum(data_frame[,i]==0)/dim(data_frame[,-1])[1])>0.5
  ...
  data_change_na=data_frame[,-1]
   data_change_na[data_change_na==0]=NA


  matplot(as.Date(data_frame$created_at),data_change_na,xlab="",ylab="Count",
        type = "l",cex.lab=1.5, cex.axis=1.5,lty=1, lwd=3,main=main_text)
  
  
  return(list("num_total_more50per_missing"=total_more50p,"percebt_user_missing"=percent_user,"user_id_more50per_missing"=user_id_more50) )
}
# zero_count_users_summary=print_column(Matrix_RRCount_per_period_user)
# zero_count_users_summary

#zero_count_users_summary=print_column_plot(Matrix_Measure_per_period,"RR Count Climiate")
#zero_count_users_summary
```
...
test_results=osninf_compare(path_for_datasets,dataset_name,first_few_days,percentage_threshold,Matrix_Measure_per_period,"TTE")
test_results$summary_table
```


```{r}
print_column_plot(Matrix_TotalOutTE_per_period,"TTE Climate")
```


```{r}
#function to graph the raw TE curve and RR Count 
#raw_TTE_RRC_plot=function(TTE_data){
# data_change_na=Matrix_RRCount_per_period[,-1]
# data_change_na[data_change_na==0]=NA
#   matplot(as.Date(Matrix_RRCount_per_period$created_at), data_change_na[,-1],xlab="",ylab="Value",cex.lab=1.5,cex.axis=1.5,type="l")
...
test_results_RRcount=osninf_compare(path_for_datasets,dataset_name,first_few_days,percentage_threshold,Matrix_Measure_per_period,"RR_Count")
test_results_RRcount$summary_table
```

```{r}
print_column_plot(Matrix_Measure_per_period,"RR Count Climate")
```
...
Matrix_RRCount_per_period=read.csv(paste0(path_for_datasets, dataset_name,"/2Matrix_RRCount_per_1D.csv"))
Matrix_Measure_per_period=Matrix_RRCount_per_period
test_results_ukraine=osninf_compare(path_for_datasets,dataset_name,first_few_days,percentage_threshold,Matrix_Measure_per_period,"RR_Count")
test_results_ukraine$summary_table
```

```{r}
print_column_plot(Matrix_Measure_per_period,"RR Count Ukraine")
```
...
test_results_ukraine_TTE=osninf_compare(path_for_datasets,dataset_name,first_few_days,percentage_threshold,Matrix_Measure_per_period,"TTE")
test_results_ukraine_TTE$summary_table
```

```{r}
print_column_plot(Matrix_Measure_per_period,"TTE Ukraine")
```
...
Matrix_RRCount_per_period=read.csv(paste0(path_for_datasets, dataset_name,"/2Matrix_RRCount_per_1D.csv"))
Matrix_Measure_per_period=Matrix_RRCount_per_period
test_results_covid19=osninf_compare(path_for_datasets,dataset_name,first_few_days,percentage_threshold,Matrix_Measure_per_period,"RR_Count")
test_results_covid19$summary_table
```

```{r}
print_column_plot(Matrix_Measure_per_period,"RR Count Covid")
```
...
```{r}
Matrix_TotalOutTE_per_period_covid19=read.csv(paste0(path_for_datasets, dataset_name,"/2Matrix_TotalOutTE_per_1D.csv"))
Matrix_Measure_per_period=Matrix_TotalOutTE_per_period_covid19
test_results_covid19_TTE=osninf_compare(path_for_datasets,dataset_name,first_few_days,percentage_threshold,Matrix_Measure_per_period,"TTE")
test_results_covid19_TTE$summary_table
```

```{r}
print_column_plot(Matrix_Measure_per_period,"TTE Covid")
```
...
```{r}
adoption_curve=adoption_volumn_fit(A_sampled_adoption_curve,"Adoption Curve")
```

```{r}
adoption_volumn_fit(volume_curve, "Volumn Curve")
```
